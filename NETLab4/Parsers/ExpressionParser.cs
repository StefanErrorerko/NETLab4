...
namespace NETLab4.ExpressionEditors
{
    public static class ExpressionParser    //??? норм шо паблік а не інтернал?
    {
        ...
        private static Component? currentNode; // ??? camel norm, почитати про конвенцію
        ...
        {
            return Regex.IsMatch(raw_part, @"^[A-z,a-z]+$");        // ??? чи норм? або розширювати Стрінг? але це крінж чи у Variable
        }
        ...
        private static void RecordValueAndCreateLeaf()  
        {
            if (part_recording)
            {
                ...
                if (currentNode.Left == null)
                    currentNode.Left = new Leaf(PartRecord(part_value));  
                else
                    currentNode.Right = new Leaf(PartRecord(part_value));    
                part_value = String.Empty;
                part_recording = false;
            }
        }
